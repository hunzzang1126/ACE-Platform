...
interface AiReplaceProps {
    ...
    canvasWidth: number;
    canvasHeight: number;
    actions: import('@/hooks/canvasTypes').CanvasEngineActions;
}
...
export function AiImageReplaceSection({ nodeId, nodeW, nodeH, canvasWidth, canvasHeight, actions }: AiReplaceProps) {
    ...
    const handleReplace = async () => {
        ...
        try {
            ...
            // Use image dimensions for generation (keeps aspect ratio)
            const genW = Math.max(nodeW, 256);
            const genH = Math.max(nodeH, 256);
            ...
            if (result.success && result.imageUrl) {
                ...
                // Replace: delete old → add new at same position/size
        ...
        <Section label="AI Replace">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    <button
                        ...
                        style={{
                            ...
                            whiteSpace: 'nowrap', opacity: (!prompt.trim() && !loading) ? 0.5 : 1,
                        }}
                    >
                        {loading ? (
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: 'spin 1s linear infinite' }}>
                ...
                <span style={{ fontSize: 10, color: '#64748b' }}>
                    Enter to generate + replace at same position
                </span>
            </div>
        </Section>
    );
