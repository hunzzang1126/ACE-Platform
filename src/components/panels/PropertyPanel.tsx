...
export function PropertyPanel({ nodes = [], selection = [], actions, selectedOverlay, onOverlayUpdate, canvasWidth = 300, canvasHeight = 250 }: Props) {
    ...
    if (!hasOverlay && !hasShape) {
        return (<div className="pp-empty"><p>Select an element to edit</p></div>);
    }
    ...
    if (hasOverlay && selectedOverlay.type === 'text') {
        return (
            <aside className="pp-root">
                <div className="pp-header"><span className="pp-title">Text</span></div>
                <Section label="Typography">
                    ...
                    <ScrubField label="Font Size" value={selectedOverlay.fontSize ?? 16} min={1} max={999} unit="px" onChange={(v) => onOverlayUpdate?.(selectedOverlay.id, { fontSize: v })} />
                    <ColorPicker label="Color" color={selectedOverlay.color || '#ffffff'} onChange={(c) => onOverlayUpdate?.(selectedOverlay.id, { color: c })} />
                    ...
                    <ScrubField label="Letter Spacing" value={selectedOverlay.letterSpacing ?? 0} min={-5} max={20} step={0.5} unit="px" onChange={(v) => onOverlayUpdate?.(selectedOverlay.id, { letterSpacing: v })} />
                    <ScrubField label="Line Height" value={selectedOverlay.lineHeight ?? 1.4} min={0.5} max={4} step={0.1} unit="x" onChange={(v) => onOverlayUpdate?.(selectedOverlay.id, { lineHeight: v })} />
                </Section>
                <AlignmentSection onAlign={alignOverlay} />
                <Section label="Opacity"><OpacitySlider value={selectedOverlay.opacity} onChange={(v) => onOverlayUpdate?.(selectedOverlay.id, { opacity: v })} /></Section>
            </aside>
        );
    }
    ...
    if (hasOverlay && selectedOverlay.type === 'image') {
        return (
            <aside className="pp-root">
                <div className="pp-header"><span className="pp-title">Image</span><span className="pp-subtitle">{selectedOverlay.fileName || 'Untitled'}</span></div>
    ...
    if (hasOverlay && selectedOverlay.type === 'video') {
        return (
            <aside className="pp-root">
                <div className="pp-header"><span className="pp-title">Video</span><span className="pp-subtitle">{selectedOverlay.fileName || 'Untitled'}</span></div>
                <Section label="Fit Mode">
                    <select className="pp-select" value={selectedOverlay.objectFit || 'cover'} onChange={(e) => onOverlayUpdate?.(selectedOverlay.id, { objectFit: e.target.value as 'cover' | 'contain' | 'fill' })}>
                        <option value="cover">Cover</option><option value="contain">Contain</option><option value="fill">Fill</option>
                    </select>
                ...
                <Section label="Playback">
                    <div className="pp-row" style={{ gap: 8 }}>
                        <label className="pp-checkbox-label"><input type="checkbox" checked={selectedOverlay.muted ?? true} onChange={(e) => onOverlayUpdate?.(selectedOverlay.id, { muted: e.target.checked })} /> Muted</label>
                        <label className="pp-checkbox-label"><input type="checkbox" checked={selectedOverlay.loop ?? true} onChange={(e) => onOverlayUpdate?.(selectedOverlay.id, { loop: e.target.checked })} /> Loop</label>
                        <label className="pp-checkbox-label"><input type="checkbox" checked={selectedOverlay.autoplay ?? true} onChange={(e) => onOverlayUpdate?.(selectedOverlay.id, { autoplay: e.target.checked })} /> Autoplay</label>
                    </div>
                </Section>
                <AlignmentSection onAlign={alignOverlay} />
                <Section label="Opacity"><OpacitySlider value={selectedOverlay.opacity} onChange={(v) => onOverlayUpdate?.(selectedOverlay.id, { opacity: v })} /></Section>
    ...
    if (hasShape && selectedNode && actions) {
        ...
        const nodeTitle = selectedNode.name || (selectedNode.type === 'text' ? 'Text' : selectedNode.type === 'image' ? 'Image' : selectedNode.type === 'path' ? 'Path' : selectedNode.type === 'ellipse' ? 'Ellipse' : selectedNode.type === 'rounded_rect' ? 'Rounded Rect' : 'Rectangle');
            ...
            <aside className="pp-root">
                ...
                {isTextNode && (
                    <Section label="Typography">
                        ...
                        <ScrubField label="Font Size" value={selectedNode.fontSize ?? 18} min={1} max={999} unit="px" onChange={(v) => actions.updateText?.(selectedNode.id, { fontSize: v })} />
                        <ColorPicker label="Text Color" color={selectedNode.color || '#000000'} onChange={(c) => actions.updateText?.(selectedNode.id, { color: c })} />
                        ...
                        <ScrubField label="Letter Spacing" value={selectedNode.letterSpacing ?? 0} min={-10} max={40} step={0.5} unit="px" onChange={(v) => actions.updateText?.(selectedNode.id, { letterSpacing: v })} />
                        <ScrubField label="Line Height" value={selectedNode.lineHeight ?? 1.4} min={0.5} max={4} step={0.05} unit="×" onChange={(v) => actions.updateText?.(selectedNode.id, { lineHeight: v })} />
                    </Section>
                )}
                ...
                <Section label="Opacity"><OpacitySlider value={selectedNode.opacity} onChange={(v) => actions.setNodeOpacity(selectedNode.id, v)} /></Section>
