...
export function EditorCanvas({ variant, canvasRef, overlayRef, engineRef, state, actions, retryInit, overlayElements = [], selectedOverlayId, onOverlaySelect, onOverlayUpdate, onOverlayDelete, onAddText, onTriggerVideoUpload, children }: Props) {
    ...
    return (
        <div className="ed-canvas-area" onMouseDown={handleCanvasAreaMouseDown} onContextMenu={(e) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY }); }} style={{ position: 'relative', overflow: 'hidden', cursor: activeTool === 'hand' ? 'grab' : activeTool === 'eyedropper' ? 'crosshair' : 'default' }}>
