...
${empty ? '★ Canvas empty → Use generate_full_design ALONE for ANY new design request. It creates the COMPLETE ad (layout + text + CTA + image). NEVER add other tools (update_element_text, add_text) alongside it. NEVER use generate_image alone for new designs.' : 'For modifications → execute_dynamic_action. For redesign → generate_full_design.'}
